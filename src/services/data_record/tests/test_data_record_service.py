...
class TestDataRecordService:
    ...
    def test_should_create_data_record(self):
        ...
        reporter = Entity(id=uuid4().hex,name='Test Reporter',entity_type='reporter')
        time = datetime.now()
        data_record = DataRecord(entity = organization,reporter=reporter,source={"phone":1234,'org':'hni'}, age = 25, time =time)
        ...
        assert loaded_data_record.reporter.entity_type == 'reporter'
        assert loaded_data_record.source['phone']==1234
